...
class MainAppTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.quiz_data = [
            {**item, "guid": quiz_guids[item["id"]]} for item in SAMPLE_DATA
        ]
    ...
    def test_check_answer_returns_correct_for_valid_answer(self):
        ...
        response = self.client.post(
            "/api/check-answer", json={"answer": question["correct_answers"][0]}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_check_answer_ignores_small_variant_when_base_image_exists(self):
        ...
        with tempfile.TemporaryDirectory() as temp_media:
            ...
            (destination_media_dir / "001_small.webp").write_bytes(
                b"optimized-duplicate"
            )
    ...
    def test_result_gallery_media_returns_200_after_quiz_completion(self):
        ...
        with tempfile.TemporaryDirectory() as temp_media:
            ...
            try:
                ...
                media_response = self.client.get(
                    f"/media/countries/{question['id']}/001.jpg"
                )
                self.assertEqual(media_response.status_code, 200)
                media_response.close()
            finally:
    ...
    def test_check_answer_returns_404_for_missing_question(self):
        ...
        response = self.client.post("/api/check-answer", json={"answer": "tokyo"})
        self.assertEqual(response.status_code, 404)
    ...
    def test_register_rejects_invalid_email_format(self):
        ...
        for email in invalid_emails:
            response = self.client.post(
                "/api/register", json={"email": email, "password": "validpass123"}
            )
            self.assertIn(
                response.status_code,
                (400,),
                msg=f"Expected 400 for '{email}', got {response.status_code}",
            )
    ...
    def test_login_after_registration(self):
        ...
        logout_resp = self.client.post(
            "/api/logout", headers={"X-CSRF-Token": csrf_token}
        )
    ...
    def test_session_cookie_secure_flag_when_enabled(self):
        ...
        try:
            ...
            client.post(
                "/api/register",
                json={"email": "secure@test.com", "password": "securepass123"},
            )
            resp = client.post(
                "/api/login",
                json={"email": "secure@test.com", "password": "securepass123"},
